...
float interceptStart = 0;
float interceptInc;
float interceptCount;
float interceptSwitch=10;
float interceptSin;
float gamma = 0;
float shrinkGrow=2;
...
void setup() {
  ...
  for( int Morph = 0; Morph < morphL; Morph ++ ) {  //inputs numerous surfii
    ...
    slope = 0;
    xCount = -240;
    if(interceptStart > 500)interceptSwitch *= -1;
    if(interceptStart < -500)interceptSwitch *= -1;
    interceptStart += interceptSwitch; 
    interceptCount = interceptStart*2/176;
    interceptInc = interceptCount;
    ...
    for(int count=0; count < holderLength; count++) {  //inputs one surface
        ...
        interXept =interceptStart-interceptInc;
        shrinkGrow+=.1;
  ...
  xCount=0;
  println("interceptInc "+interceptInc);
};
