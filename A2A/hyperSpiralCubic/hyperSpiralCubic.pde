...
void draw() {
  ...
  if( xCount < 1) {  //loads the array until the limit is reached
    ...
    for(int turn=0; turn<squirle; turn+=1 ) {
      ...
      for(int angle = 0;angle < spiraLength; angle += 1) {
        ...
        holder[turn][inc][0] = complex*.01;//scaled for display
        holder[turn][inc][1] = y*6;
        holder[turn][inc][2] = z*.01;//x here in place of the complex term will give the "saddle shape"
        //  println("inc: " + inc);0
        rad=rad+.0014;
        inc +=1;
      }
  ...
  //displays the complete set of parabolas showing complex surface
    ...
    for(int drawIt = 0; drawIt < spiraLength; drawIt ++) {
      vertex(holder[yCount][drawIt][0], holder[yCount][drawIt][1], holder[yCount][drawIt][2]);
    }
    endShape();
