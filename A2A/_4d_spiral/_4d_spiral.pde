...
int yCount=0;
int squirle=360;
int spiraLength=2000;

float[][][] holder = new float[squirle][spiraLength][4];
 ...
 //  line(0,0,-200,0,0,200);
  ...
  if( xCount < 1) {  //loads the array until the limit is reached
    ...
    for(int turn=0; turn<squirle; turn+=1 ) {
      ...
      for(int angle = 0;angle < spiraLength; angle += 1) {
        ...
        holder[turn][inc][0] = x*40;//scaled for display
        holder[turn][inc][1] = y*40;
        holder[turn][inc][2] = z*8;
        holder[turn][inc][3] = complex*8;
        //  println("inc: " + inc);0
        rad=rad+.0014;
        inc +=1;
      }
  ...
  //displays the complete set of parabolas showing complex surface
    ...
    for(int drawIt = 0; drawIt < spiraLength; drawIt ++) {
      vertex(holder[yCount][drawIt][3], holder[yCount][drawIt][1], holder[yCount][drawIt][2]);
    }
    endShape();
