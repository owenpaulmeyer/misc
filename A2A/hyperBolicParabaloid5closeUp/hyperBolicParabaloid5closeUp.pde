...
int begin = -8;           //start of plotting range
int beginArray = begin*-1;
int end = 8;              //end of plotting range
float pointScale = .01;
float timeScale = .5;
int holderLength = int((end-begin)/timeScale);
int xCount = 0;
...
void draw() {
  ...
  if( x < end) {  //loads the array until the limit is reached
    for(float pointInc = begin; pointInc < end; pointInc += .001 ) {
      y = pointInc;
      ...
      holder[xCount][int(pointInc + beginArray)][0] = y*2;//scaled for display
      holder[xCount][int(pointInc + beginArray)][1] = z*2;
      holder[xCount][int(pointInc + beginArray)][2] = x*2;//complex;//x here in place of the complex term will give the "saddle shape"
    }
  }
  ...
  for(int idx=0; idx< holderLength; idx ++) {
    beginShape();
    for(int drawIt = 0; drawIt < holderLength-16; drawIt ++) {
