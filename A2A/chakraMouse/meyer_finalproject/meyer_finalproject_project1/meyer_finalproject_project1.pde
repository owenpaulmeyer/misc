...
block cube_1 = new block( 40, 300, 10,40,00,00 );
;
...
cross_section[] cross_sections_array = new cross_section[ 200 ];
...
void draw() {
 ...
 cube_1.rotation( radians( 5.03 ),0,0, "y" );
 cube_1.rotation( radians( -1.3 ), cube_1.center( "x" ), cube_1.center( "z" ), "y" );
   
  
  cross_section holder = new cross_section( cube_1 );
  
  
  cross_sections_array[ 200- (snake_head % 200)-1 ] = holder;
 
color phil = color( 27, 100, 188, 120 );

if(snake_head > 200){
  
int snake_head_pass = snake_head;
  for( int idxx = 0; idxx < 600 ; idxx += 3 ) {
    snake_head_pass ++;
    cross_now = cross_sections_array[ 200 -(snake_head_pass % 200)-1 ]; 
//    noStroke();
    stroke( 127, 216, 237 );
    fill( r, g, b, 120 );
    strokeWeight( 1 );
    ...
    vertex( idxx-300  , cross_now.section_edges[ idx+1 ], cross_now.section_edges[ idx+2 ] );
    
    }

    endShape( CLOSE );
//    phil = color( 27, 100, 188, 190 );
    rotateZ( radians( .2 ) );
    rotateX( radians( .45 ) );
