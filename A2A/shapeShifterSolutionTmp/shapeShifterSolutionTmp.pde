...
float[][][][] holder = new float[morphL][holderLine][holderLength][3];
...
void setup() {
  ...
  for( int Morph = 0; Morph < morphL; Morph ++ ) {
    ...
    x = time;//the x term(axis)of the function is used as "time" as the complex term is not constrainable
      ...
      for(int count=0; count < holderLine; count++) {
      ...
      for(float pointInc = begin; pointInc < end; pointInc += 1 ) {
        ...
        holder[Morph][count][int(pointInc + beginArray)][0] = complex*.0001;//scaled for display
...
void displaySurf() {
  ...
  for(int morph =0; morph < morphL; morph++) {
    for(int idx=0; idx< holderLine; idx ++) {
      beginShape();
      for(int drawIt = 0; drawIt < holderLine; drawIt ++) {
