...
void draw() {
  ...
  if( x < 20) {
    for(float pointInc = begin; pointInc < end; pointInc += pointScale ) {
      ...
      holder[int(pointInc + beginArray)][0] = y;
      holder[int(pointInc + beginArray)][1] = z;
      holder[int(pointInc + beginArray)][2] = complex;
    }
  }
  for(int idx=0; idx< 40; idx ++) {
    ...
    for(int drawIt = 0; drawIt < holder.length; drawIt ++) {
      ...
      point(holder[drawIt][0], holder[drawIt][1], holder[drawIt][2]);
    }
