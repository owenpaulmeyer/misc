...
void draw() {
  ...
  if( x < end) {  //loads the array until the limit is reached
    for(float pointInc = begin; pointInc < end; pointInc += pointScale ) {
      ...
      complex = -(y*y*y)+(3*x*x*y); //complex term of the function sans i; just used as the third axis

      //array to load set of parabolas into; xCount is for a single parabola
      holder[xCount][int(pointInc + beginArray)][0] = complex*.0001;//scaled for display
      holder[xCount][int(pointInc + beginArray)][1] = y*1;
      holder[xCount][int(pointInc + beginArray)][2] = z*.0001;//x here in place of the complex term will give the "saddle shape"
    }
  }
