...
float [] coefPass = {0,0,0,0,1 };  //the first item in the list is the y intercept
                                   //the last item is the x coefficient of the hightest
                                  //exponent. 0's must be used for x's not present
float xStart = -10;  //the first x input value
float xEnd = 10;  //the last x input value
float xInc = .01;  //x is iterated by this amount
float xScale = 50;  //scales what is displayed (x values)
