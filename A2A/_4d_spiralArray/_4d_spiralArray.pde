...
float[][][][] holder = new float[180][squirle][spiraLength][4];
...
void draw() {
  ...
  if( xCount < 1) {  //loads the array until the limit is reached
    ...
    //    stroke(25, 252, 232); 
    //  }
    for(int lupe=0; lupe < 180; lupe++ ) {

      for(int turn=0; turn<squirle; turn+=1 ) {
        inc=0;
        rad=0;
        for(int angle = 0;angle < spiraLength; angle += 1) {

          x = cos( (angle+turn*10)*PI/180 ) *rad;
          y = sin( (angle+turn*10)*PI/180 ) *rad;
          ...
          complex = pow(-y,3)+3*x*x*y; //complex term of the function sans i; just used as the third axis

          //array to load set of parabolas into; xCount is for a single parabola
          holder[lupe][turn][inc][0] = x*20;//scaled for display
          holder[lupe][turn][inc][1] = y*20;
          holder[lupe][turn][inc][2] = z*.08;
          holder[lupe][turn][inc][3] = complex*.08;
          //  println("inc: " + inc);0
          rad=rad+.0014;
          inc +=1;
        }
  ...
  for(int lupes = 0; lupes < 180; lupes++ ) {

    beginShape();

    for(int drawIt = 0; drawIt < spiraLength; drawIt ++) {
      vertex(holder[lupes][yCount][drawIt][3], holder[lupes][yCount][drawIt][1], holder[lupes][yCount][drawIt][2]);
    }
    endShape();
  }
...
void rotation(float theta, int axisA, int axisB) {
  for(int luper = 0; luper < 180; luper ++) {

    for(int surface=0; surface < squirle; surface++) {
      for(int lines=0; lines < spiraLength; lines++) {

        float pass_x = ( holder[luper][ surface ][ lines ][ axisA ] * cos( theta ) )     
          + ( holder[luper][ surface ][ lines ][ axisB ] * -sin( theta ) );       


        float pass_y = ( holder[luper][ surface ][ lines ][ axisA ] * sin( theta ) )     
          + ( holder[luper][ surface ][ lines ][ axisB ] *  cos( theta ) );

        holder[luper][ surface ][ lines ][ axisA ] = pass_x;
        holder[luper][ surface ][ lines ][ axisB ] = pass_y;
      }
    }
  }
}
