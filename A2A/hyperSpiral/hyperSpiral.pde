  ...
  if( xCount < 1) {  //loads the array until the limit is reached
    ...
    for(int turn=0; turn<squirle; turn++ ) {
      ...
      for(int angle = 0;angle < spiraLength; angle += 1) {
        ...
        holder[turn][inc][0] = complex*.05;//scaled for display
        holder[turn][inc][1] = y*2;
        holder[turn][inc][2] = z*.05;//x here in place of the complex term will give the "saddle shape"
        //  println("inc: " + inc);0
        rad=rad+.01;
        inc ++;
      }
