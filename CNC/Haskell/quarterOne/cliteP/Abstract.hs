...
data Program      =  Prog Decls Stmts deriving Show
type Decls        =  [Decl]
...
data Stype        =  Skip |
					 ...
					 As Assnmt |
					 Co Condl	|
					 Lo Loop  deriving Show

type Block        =  Stmts
data Condl        =  Condl Expr Stmt Stmt  deriving Show-- test, then branch, else branch
data Assnmt       =  Assnmt References Expr  deriving Show-- target; source
data Loop         =  Loop Expr Stmt  deriving Show-- test; body
...
data Var          =  Var String
data References   =  Variable String | ArrayRef String Expr deriving Show--Expr as array index
data Values       =  I IntVal | F FloatVal | B BoolVal | C CharVal deriving Show
type IntVal       =  Int
type FloatVal     =  Float
type BoolVal      =  Bool
type CharVal      =  Char
