...
block cube_1 = new block( 200, 60, 60,50,00,00 );
;
...
cross_section[] cross_sections_array = new cross_section[ 100 ];
...
void draw() {
 ...
 cube_1.rotation( radians( -1.8 ), cube_1.center( "x" ), cube_1.center( "y" ), "z" );
   
  
  cross_section holder = new cross_section( cube_1 );
  
  
  cross_sections_array[ snake_head % 100 ] = holder;
 


if(snake_head > 100){
int snake_head_pass = snake_head;
  for( int idxx = 0; idxx < 500 ; idxx += 5 ) {
    snake_head_pass ++;
    cross_now = cross_sections_array[ snake_head_pass % 100 ]; 
//    noStroke();
    stroke( 58, 216, 50 );
    fill( 21, 26, 234, 199 );
    strokeWeight( 2 );
...
//    strokeWeight( 2 );
//    beginShape();
//    for( int id = 0; id < holder.section_edges.length -1; id += 3 ) {
//    vertex( idxx, cross_now.section_edges[ id+1 ], cross_now.section_edges[ id+2 ] );
//    }
//    endShape( CLOSE );
}
