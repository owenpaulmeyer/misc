...
cross_section[] cross_sections_array = new cross_section[ 400  ];
...
void draw() {
 ...
 cube_1.rotation( radians( -1.1 ), cube_1.center( "x" ), cube_1.center( "y" ), "z" );
  ...
  cross_sections_array[ snake_head % 400 ] = holder;
...
int snake_head_pass = snake_head;
  for( int idxx = 0; idxx < 600 ; idxx += 10 ) {
    snake_head_pass += 6;
    cross_now = cross_sections_array[ snake_head_pass % 400 ]; 
    noStroke();
//      stroke( 180 );
...
//    }
    ...
    vertex( idxx-400  , cross_now.section_edges[ idx+1 ], cross_now.section_edges[ idx+2 ] );
