...
block cube_1 = new block( 120, 120, 120,100,00,00 );
;
...
ArrayList cross_sections_list = new ArrayList();
...
void draw() {
 ...
 strokeWeight( 5 );
  ...
  cross_sections_list.add( holder );
 
 println( cross_sections_list.size() );



  for( int idxx = 0; idxx < cross_sections_list.size(); idxx ++ ) {
    cross_now = ( cross_section ) cross_sections_list.get( idxx ); 
    stroke( 58, 216, 50 );
    fill( 21, 26, 234, 199 );
    strokeWeight( 2 );
    beginShape( TRIANGLE_FAN );
    for( int idx = 0; idx < cross_now.section_edges.length -1; idx += 3 ) {
    vertex( idxx, cross_now.section_edges[ idx+1 ], cross_now.section_edges[ idx+2 ] );
    }
    println( idxx );
    endShape( CLOSE );
 ...
 cube_1.rotation( radians( .4 ),0,0, "x" );
 cube_1.rotation( radians( -1.8 ), cube_1.center( "" ), cube_1.center( "y" ), "z" );
...
// cube_1.display();
  ...
  stroke( 21, 26, 234 );
    ...
    strokeWeight( 2 );
    beginShape();
    for( int idx = 0; idx < holder.section_edges.length -1; idx += 3 ) {
    vertex( holder.section_edges[ idx   ], holder.section_edges[ idx+1 ], holder.section_edges[ idx+2 ] );
    }
    endShape( CLOSE );
