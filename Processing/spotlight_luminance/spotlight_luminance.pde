...
void setup() {
  photo = loadImage( "test.jpg" );  //loads the image "test" into the variable "photo"
...
void draw() {
  ...
  for (int x = 0; x < photo.width; x++ ) {  //nested loop; cycles through all of the xy coordinates of the image
    for (int y = 0; y < photo.height; y++ ) {
      ...
      float spot_radius = 200;  //sets the radius of the negative spotlight
      ...
      else if (distance < spot_radius*2){  //this is for the area from the spotlight rim to double the spot radius
