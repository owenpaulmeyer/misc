...
float[][][] holderT = new float[vale][121][4];
...
void setup() {
  ...
  xCount = 0;
  populate(holderB);
  slitScan(holderA,holderS);
  slitScan(holderS,holderT);
  // rotationB( -.8,3,0);
};
...
rotation( -.01,2,3,holderT);
   rotation( -.01,0,3,holderT);
...
void display(float[][][] holder) {
  ...
  for(int idx=0; idx< vale-1; idx +=1) {
...
void slitScan(float[][][] holder,float[][][] newHolder) {
 int countey = 0;
...
//      }
      ...
      for(int valences = 0; valences < 19; valences++) {//one slit
        ...
        newHolder[valences][phase][0] = holder[valences][phase][0];
        newHolder[valences][phase][1] = holder[valences][phase][1];
        newHolder[valences][phase][2] = holder[valences][phase][2];
        newHolder[valences][phase][3] = holder[valences][phase][3];
        
        
        
      }
      
      rotation(.1049,2,3,holder);
