...
class GNode {
  ...
  void change ( WeightedEdge e, Ratio w ) {
    
    //Ratio w = new Ratio( scale, scale );
  ...
  void shift ( Edge e, float w ) {
    
    if     ( e.isNorth()  ) north.shift( w );
    else if( e.isSouth()  ) south.shift( w );
    else if( e.isWest()   ) west.shift( w );
    else if( e.isEast()   ) east.shift( w );
    else if( e.isNorthW() ) northwest.shift( w );
    else if( e.isNorthE() ) northeast.shift( w );
    else if( e.isSouthW() ) southwest.shift( w );
    else if( e.isSouthE() ) southeast.shift( w );
  }
...
//a buffered result from setting teeth to a location in the grid
