    ...
    for ( Edge edge : adjs ) {
      ...
      */
      ...
      for ( Edge e : directions( ) ) {
        if ( !e.equals( edge ) && adjs.contains( e ) ) {
          Point p = new Point( e );
          tooth.addPoint( p );
        }
        //else if ( !e.equals( edge ) ) {
        //  tooth.addPoint( new Point( e, -1 ) );
        //}
      }
  ...
  */
  ...
  void display( int scale, int xS, int yS ){
    ...
    for ( Location l : set ) {
      ...
      float _y = yS + l.yloc( ) * scale;
      for ( Edge e : n.adjacents( ) ) {
        float _xto = _x + e.x( )*scale/2.2;
        float _yto = _y + e.y( )*scale/2.2;
        line( _x, _y, _xto, _yto  );
      }
      ...
      strokeWeight( 1.5 );
      stroke( 0, 0 , 0 );
      ellipse( _x, _y, .25 * scale, .25 * scale );
      strokeWeight( sW );
...
class Location {
  ...
  Location trace ( Location loc ) {
    return new Location( xloc( ) + loc.xloc( ), yloc( ) + loc.yloc( ) );
  }
